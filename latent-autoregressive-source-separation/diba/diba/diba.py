...
def _ancestral_sample(
    ...
    for sample_t in tqdm(range(sample_tokens)):
        ...
        if ll_coords.numel() > 0:
            ...
            posterior_data = _compute_log_posterior(ll_data, ll_coords, log_p_0, log_p_1)
            if sample_t < 2:
                print(f'SHAPE OF posterior_data: {posterior_data.shape}') #torch.Size([100, 16521])
                print(f'SHAPE OF log_post_sum: {log_post_sum.shape}') # torch.Size([100])
                print(f'n_samples: {n_samples}')
                print(f'SHAPE OF ll_coords: {ll_coords.shape}')#torch.Size([2, 16521])
                print(f'SHAPE OF THE SUM log_post_sum + posterior_data: {(log_post_sum + posterior_data).shape}')#torch.Size([100, nnz])
            ...
            if sample_t < 2:
                print(f'beams shape: {beams.shape}, coords_idx shape: {coords_idx.shape}') #100, 100
                print(f'after topk log_post_sum: {log_post_sum.shape}')# torch.Size([100,1])
    ...
    print(f'num_current_beams: {num_current_beams}')
...
def fast_sampled_separation(
    ...
    for sample_t in tqdm(range(sample_tokens)):
        ...
        # Note: x_0, x_1 have shape (n_sample,)
        xs_0[:, sample_t + 1] = x_0.clone()
        xs_1[:, sample_t + 1] = x_1.clone()
