...
class TrackPairsDataset(SeparationDataset):
    def __init__(
        ...
        # get filenames
        dir_1_files = sorted([Path(f).name for f in dir_1_files])
        dir_2_files = sorted([Path(f).name for f in dir_2_files])

        ###################################################################
        instrument_1 = []
        instrument_2 = []
        for file_name_1 in dir_1_files:

            # since we are using the same files as train/test the .wav files are interleaved by .dur files
            if file_name_1[-4] == ".dur":
                continue
        
            instrument_1.append(file_name_1)  

        for file_name_2 in dir_2_files:

            # since we are using the same files as train/test the .wav files are interleaved by .dur files
            if file_name_2[-4] == ".dur":
                continue

            instrument_2.append(file_name_2)    


        assert len(instrument_1) == len(instrument_2), f'instrument_1:{len(instrument_1)} is different from instrument_2:{len(instrument_2)}'
        self.filenames = list(zip(instrument_1, instrument_2))

        ###################################################################      

        print(f"Found {len(instrument_1)} instances of instrument_1")
        print(f"Found {len(instrument_2)} instances of instrument_2")

        if len(self.filenames) != len(instrument_1):
            unused_tracks = len(dir_1_files.difference(self.filenames))
            warnings.warn(
                f"Not using all available tracks in {self.dir_1} ({unused_tracks})"
            )

        if len(self.filenames) != len(instrument_2):
...
class ChunkedPairsDataset(TrackPairsDataset):
    def __init__(
        ...
        random_shuffler = random.Random(42)
        for file in self.filenames:
            ...
            #shuffle and then take one chunk per track
            random_shuffler.shuffle(available_chunks)
            available_chunks = available_chunks[:1]
            print(f'available_chunks for {file}: {len(available_chunks)}')
