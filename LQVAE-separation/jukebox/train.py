...
def train(model, orig_model, opt, shd, scalar, ema, logger, metrics, data_processor, hps):
    ...
    print_once(f'Total number of iterations: {hps.save_iters}')
    for i, x in logger.get_range(data_processor.train_loader):
        ...
        # Save checkpoint
        with t.no_grad():
            if hps.save and (logger.iters % hps.save_iters == 0 or finished_training):
                if ema is not None: ema.swap()
                orig_model.eval()
...
def run(hps="teeny", port=29501, **kwargs):
    ...
    wandb.init(
        ...
        name="run_prior_drums",
        # track hyperparameters and run metadata
        config={
        "learning_rate": hps.lr,
        "model": "prior",
        "dataset": "Slakh2100-Drums&Bass",
        "train-size": len(data_processor.train_dataset),
        "test-size": len(data_processor.test_dataset),
        "epochs": 5,
        "sr": hps.sr,
        "bs": hps.bs,
        "levels": hps.levels,
...
# mpiexec -n 1 python ./jukebox/train.py --hps=vqvae --sample_length=131072 --bs=4 --audio_files_dir=../../train/mix --labels=False --train --test --aug_shift --aug_blend --name=lq_vae --test_audio_files_dir=../../test/mix
    # Run training, eval, sample
    for epoch in range(hps.curr_epoch, hps.epochs):
        print_once(f'Current Epoch: {epoch} - Epochs: {hps.epochs}')
        metrics.reset()
        ...
        if hps.train:
            ...
            wandb.log({'train-metrics': train_metrics}, step=epoch)
            if rank == 0:
        ...
        if hps.test:
            ...
            wandb.log({'test-metrics': test_metrics}, step=epoch)
            if rank == 0:
